(function () {
  ...
  function init() {
    // Small delay to ensure all layout is calculated
    requestAnimationFrame(function() {
      requestAnimationFrame(function() {
        var activeSection = document.body.getAttribute('data-active-section');
        if (!activeSection) return;

        // Find the navbar link matching the active section
        var navLink = document.querySelector('.navbar__links a[data-section="' + activeSection + '"]');
        if (!navLink) return;

        // Draw persistent circle around it
        drawPersistentCircle(navLink);
      });
    });
  }
  ...
  function removeCircle() {
    var overlay = document.getElementById('navbar-circle-overlay');
    if (!overlay) return;
    var existingCircle = document.getElementById('persistent-circle');
    if (existingCircle) {
      overlay.removeChild(existingCircle);
    }
  }
  ...
  function drawPersistentCircle(link) {
    // Check if the link is actually visible (important for mobile when menu is closed)
    var style = window.getComputedStyle(link.parentElement);
    if (style.display === 'none' || !link.offsetParent) {
      // Element is hidden, don't draw circle
      removeCircle();
      return;
    }
    ...
    if (rect.width === 0 && rect.height === 0) {
      // Element has no dimensions, don't draw circle
      removeCircle();
      return;
    }
    ...
    // Create or get the SVG overlay
    var overlay = document.getElementById('navbar-circle-overlay');
    if (!overlay) {
      overlay = document.createElementNS(ns, 'svg');
      overlay.id = 'navbar-circle-overlay';

      // Insert at the beginning of body to ensure it covers the whole document
    ...
    // Update SVG dimensions to match document size
    ...
    overlay.style.cssText = 'position:absolute;top:0;left:0;pointer-events:none;z-index:9999;overflow:visible';
    overlay.setAttribute('width', docWidth);
    overlay.setAttribute('height', docHeight);
    ...
    // Clear existing circles (keep defs)
    var existingCircle = document.getElementById('persistent-circle');
    if (existingCircle) {
      overlay.removeChild(existingCircle);
    }
    ...
    var pad = 10;
    var cx = rect.left + scrollLeft + rect.width / 2;
    var cy = rect.top + scrollTop + rect.height / 2;
    var rx = Math.max(rect.width / 2 + pad, 32);
    var ry = Math.max(rect.height / 2 + pad, 14);

    var group = document.createElementNS(ns, 'g');
    group.setAttribute('id', 'persistent-circle');

    // Layer 1: Background dust/blur
  ...
  // Redraw circle on window resize to keep it aligned
  var resizeTimeout;
  window.addEventListener('resize', function() {
    ...
    resizeTimeout = setTimeout(function() {
      ...
      if (!activeSection) return;
      var navLink = document.querySelector('.navbar__links a[data-section="' + activeSection + '"]');
      if (navLink) drawPersistentCircle(navLink);
    }, 100);
  });

  // Redraw circle when mobile menu is toggled
  ...
  if (burger) {
    burger.addEventListener('click', function() {
      setTimeout(function() {
        ...
        var navLink = document.querySelector('.navbar__links a[data-section="' + activeSection + '"]');
        if (navLink) drawPersistentCircle(navLink);
      }, 50); // Small delay to ensure menu animation completes
    });
  }
}());
